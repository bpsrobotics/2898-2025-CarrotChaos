...
class AlignOdometry(val apriltagId : Int, var targetPose2d : Pose2d = Pose2d(3.1, 4.24, Rotation2d(0.0)), val maxSpeed : Double = 1.5, val maxRotSpeed : Double = 1.0) : Command() {
    ...
    override fun execute() {
        ...
        if(xSpeed.absoluteValue < deadzone) xSpeed = 0.0
        else xSpeed += ks * xSpeed.sign
//
        if(ySpeed.absoluteValue < deadzone) ySpeed = 0.0
        else ySpeed += ks * ySpeed.sign
//
        ...
        if(totalError < 0.2) {
            when {
                xPID.error > yPID.error && xPID.error > rotationPID.error * 2 -> {ySpeed = 0.0; rotationSpeed = 0.0}
                yPID.error > xPID.error && yPID.error > rotationPID.error * 2 -> {xSpeed = 0.0; rotationSpeed = 0.0}
                else -> {xSpeed = 0.0 ; ySpeed = 0.0}
            }
        }
