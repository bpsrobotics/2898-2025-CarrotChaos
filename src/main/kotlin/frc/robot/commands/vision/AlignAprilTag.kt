...
class AlignAprilTag(val apriltagId : Int, var yToTag : Double = 0.0) : Command() {
    ...
    override fun execute() {
        ...
        else xSpeed += deadzone * xSpeed.sign
        if(ySpeed.absoluteValue < deadzone) ySpeed = 0.0
        else ySpeed += deadzone * ySpeed.sign
    ...
    override fun isFinished(): Boolean {
        ...
        if(desiredTag != null && yPID.error < 0.02 && rotationPID.error < 0.02 && timer.hasElapsed(2.0) && desiredTag != lastDesiredTag) trueFrames += 1
        else trueFrames = 0
